...
## 5.5 Quinta tanda (5/9/2026) — 3 patrones portados desde MOTORMAX/RPM Manager

Después de comparar este proyecto contra `MOTORMAX-crm` (ver su propio `MAPEO_ARQUITECTURA.md`, sección 5), se evaluaron 9 patrones candidatos y se descartaron/diferenciaron 6 por no aportar valor real acá (cliente favorito: redundante con `prioridad`, que ya existe; consentimiento de notificaciones: ya cubierto por el gate de Términos y Condiciones; reporte de agrupación: sin caso de uso claro todavía; temperatura de prospecto y motivos de cierre con reapertura: se dejan pendientes junto con el resto de Prospectos, no es prioridad hoy). Se implementaron los otros 3:

1. **Log de auditoría por cliente (`EVENTOS`, `LM_EVENTOS_V1`), append-only.** Array nuevo, nunca se edita ni se borra una entrada ya creada — solo se agregan. `registrarEvento(clienteId, tipo, detalle)` la alimenta desde 4 puntos: alta de cliente, cambio de prioridad (individual y en lote), cerrar/reabrir cliente (individual y en lote), y resolución de una tarea vinculada a un cliente (ver punto 3). En la ficha del cliente aparece un botón "📜 Historial" (abre `#modal-historial-cliente`, lista simple ordenada por fecha descendente) y una línea chica "Última modificación: dd/mm/aaaa hh:mm" debajo del nombre — mismo estilo visual que "Último contacto" (texto secundario, no destacado).
2. **Aviso de posible duplicado (no bloqueante).** `detectarDuplicados()` compara teléfono (solo dígitos) y nombre (case-insensitive) entre todos los clientes; si coincide con algún otro, aparece un ⚠️ con tooltip al lado del nombre en la tabla de Clientes. Nunca impide guardar ni cargar — es solo una señal visual. Esperable más falsos positivos que en un rubro notarial (un contador puede tener el mismo teléfono de contacto en 2-3 empresas del mismo dueño), aceptado a propósito por eso mismo ser no bloqueante.
3. **Constancia al resolver una tarea — 100% opcional acá (a diferencia de CRM Notarial, ver su propio mapeo).** Al hacer click en "Resolver" (antes tocaba directo "✓ Hecha") se abre `#modal-resolver-tarea`: sí/no + un textarea de detalle, nunca obligatorio, nunca bloquea el guardado aunque quede vacío — decisión explícita del usuario por el volumen de tareas de un estudio contable (mucho mayor al de una escribanía). Si la tarea está vinculada a un cliente, el detalle (si se cargó) se agrega también como evento al log de auditoría de ese cliente. La fila de la tarea muestra "✓ Hecha" / "✗ No hecha" + el detalle inline, sin abrir nada, apenas queda resuelta.
